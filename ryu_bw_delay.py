...
class ProjectController(app_manager.RyuApp):
    ...
    def ping_packet_handler(self, pkt):
        ...
        s1 = info[0]
        s2 = info[1]
        if int(s2)!=0 and s1 != s2:
            avg_controller_delay = (self.delay_matrix[int(s1)][0]/1000/2 + self.delay_matrix[int(s2)][0]/1000/2)/2
            latency = (time.time() - float(info[2]) - avg_controller_delay) * 1000  # in ms
        else:
            latency = (time.time() - float(info[2])) * 1000
        # print "s%s to s%s latency = %f ms" % (s1, s2, latency)
        self.delay_matrix[int(s1)][int(s2)] = round(latency,5)
        self.delay[int(s1)][int(s2)] = round(latency,5)
    ...
    def dijkstra(self, graph, node_start, node_end=None):
        ...
        while len(Q) > 0:
            ...
            for p in self.switches:
                # print 'graph[%s] = %s' %(u,graph[u])
                if p in graph[u]:
                    if graph[u][p] != None:
                        # print 'graph[u] enter: %s' %graph[u]
                        w = 10.0/self.rbw_matrix[u][p] # Cost Function
                        # w = self.delay[u][p]
    ...
    def get_paths(self, src, dst, first_port, final_port, max_k=2):
        ...
        # Yen's first step is find the first shortest path using dijkstra.
        ...
        try:
            ...
            # print "node_spur=", node_spur, " path_root=", path_root
                    ...
                    # print 'link[%s][%s] is removed' %(curr_path[i],curr_path[i+1])
        ...
        print "YenKSP->"
        for path_k in A:
            print 'Path[ (host %s) --> (host %s) ] : %s' % (src,
                                                            dst, str(path_k))
            tmp.append(path_k)
    ...
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst):
        ...
        paths = self.get_paths(src, dst, first_port, last_port)
        pw = []
        for path in paths:
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,
                                             actions)]
        if buffer_id:
            mod = parser.OFPFlowMod(datapath=datapath,buffer_id=buffer_id,idle_timeout=hard_timeout,
                                    priority=priority, match=match,
                                    instructions=inst)
        else:
            mod = parser.OFPFlowMod(datapath=datapath,idle_timeout=hard_timeout,priority=priority,
